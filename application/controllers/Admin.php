...
class Admin extends CI_Controller
{
    ...
    public function listla()
    {
        $data['title'] = 'List Laporan Akhir';;
        $data['all_data'] = $this->ta->getLA();
        $data['user'] = $this->user->getUserData();
        $data['all_user'] = $this->user->getUserDataAll();

        if ($this->input->post('keyword')) {
            $data['all_data']  = $this->admin->searchUserData();
        }
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('admin/listla', $data);
        $this->load->view('templates/footer');
    }

    public function listsi()
    {
        $data['title'] = 'List Skirpsi';;
        $data['all_data'] = $this->ta->getSI();
        $data['user'] = $this->user->getUserData();
        $data['all_user'] = $this->user->getUserDataAll();
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('admin/listsi', $data);
        $this->load->view('templates/footer');
    }
